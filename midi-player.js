...
class MIDIPlayer {
    ...
    async play(startTime = 0) {
        ...
        try {
            ...
            console.log('✅ Воспроизведение начал��сь');
    ...
    async exportToWAV(onProgress) {
        ...
        try {
            ...
            // ✅ ПРАВИЛЬНЫЙ СПОСОБ: Используем прямое планирование в оффлайн контексте
            // Планируем ноты напрямую
            notesToPlay.forEach((noteData, index) => {
                ...
                // Используем абсолютное время в оффлайн контексте
                const absoluteTime = noteData.time;
                
                // Планируем ноту напрямую
                renderSynth.triggerAttackRelease(
                    noteName, 
                    noteData.duration, 
                    absoluteTime, 
                    normalizedVelocity
            ...
            console.log('📊 Buffer length:', renderedBuffer.length);
            console.log('📊 Buffer duration:', renderedBuffer.duration);
